...
int in, out; //取值范围在 0,1, ... , n-1;
int counter; //取值范围在 0,1, ... , n;
\end{minted}
\end{frame}
...
Swait(s1,s2,...,sn)
  if s1 >= 1 and ... and sn >= 1 then
    ...
    with the first si found with si<1, and 
    set the program count of this process to 
...
\linespread{1}
\begin{minted}{pascal}
var forks: array[0, ..., 4] of semaphore;
repeat
  wait(forks[i]);
...
\linespread{1}
\begin{minted}{pascal}
var forks: array[0, ..., 4] of semaphore:=(1,1,1,1,1);
processi
  repeat
    think;
    Sswait(forks[(i+1) mod 5],forks[i]);
    eat
    Ssignal(forks[(i+1) mod 5],forks[i]);
  until false
\end{minted}
\end{frame}
...
\begin{frame}[fragile]{思考}
  \begin{easylist} \easyitem
    & difference是一个整数类型的变量，需不需要加互斥信号量对其实施互斥访问。
    && 注意一个进程和一类进程的差异。
    && 多个进程对同一资源进行访问时，才需要保护。
  \end{easylist}
\end{frame}
